...
def state(m: Machine, ignore: range = range(0)) -> dict:
...
def assert_equivalent(src: str, states: int = 40, entry: str = "", other: str = "",
    ...
    for _ in range(states):
        ...
        before = state(run(entry + src + other, random.Random(seed)), ignore)
        after = state(run(entry + out + other, random.Random(seed)), ignore)
